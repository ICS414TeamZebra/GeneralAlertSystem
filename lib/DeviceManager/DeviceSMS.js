...
class DeviceSMS extends DeviceGeneric {
  static open() {
    console.log('Connected to SMS server.');
  }
  ...
  static test(strTest, strResult) {
    for (const location of locations) {
      console.log(`Texting people in ${names.locations[location]}`);
      for (const number of testNumbers[location]) {
        console.log(`\t- Sent SMS saying "${message}" to ${number}`);
      }
    }
  }
  ...
  static close() {
    console.log('Disconnected from SMS server.');
  }
}
